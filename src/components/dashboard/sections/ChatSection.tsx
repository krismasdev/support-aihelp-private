...
export const ChatSection = ({ userProfile }: ChatSectionProps) => {
  ...
  const handleHelperSwitch = async (helper: any) => {
    ...
    try {
      ...
      dispatch(fetchHelpers(userId) as any); // Refresh helpers to update isDefault
    } catch (error) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2">
            {helpers.map((helper) => (
              <Button
                ...
                className={`flex flex-col items-center gap-1 h-auto p-3 transition-all ${
                  activeHelper?.id === helper.id 
              ...
              >
                ...
                <span className="text-xs font-medium text-center leading-tight">{helper.name}</span>
